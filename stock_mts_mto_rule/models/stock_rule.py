...
class StockRule(models.Model):
    ...
    def get_mto_qty_to_order(self, rule, product, product_qty, product_uom, values):
         
        precision = self.env['decimal.precision']\
            .precision_get('Product Unit of Measure')

        src_location_id = rule.mts_rule_id.location_src_id.id
    ...
    def _run_split_procurement(self, procurements):
        ...
        for procurement, rule in procurements:
            needed_qty = self.get_mto_qty_to_order(rule, procurement.product_id, procurement.product_qty,
                                                procurement.product_uom, procurement.values)

            if float_is_zero(needed_qty, precision_digits=precision):
                getattr(rule.mts_rule_id, '_run_%s' % rule.mts_rule_id.action)([(procurement, rule.mts_rule_id)])
            elif float_compare(needed_qty, procurement.product_qty,
                            precision_digits=precision) == 0.0:
                getattr(rule.mto_rule_id, '_run_%s' % rule.mto_rule_id.action)([(procurement, rule.mto_rule_id)])
            else:
                mts_procurement = self.env['procurement.group']\
                    .Procurement(procurement.product_id, procurement.product_qty-needed_qty, procurement.product_uom,
                                 procurement.location_id, procurement.name + '-mts', procurement.origin, procurement.company_id,
                                 procurement.values)

                getattr(rule.mts_rule_id, '_run_%s' % rule.mts_rule_id.action)([(mts_procurement, rule.mts_rule_id)])
                mto_procurement = self.env['procurement.group']\
                    .Procurement(procurement.product_id, needed_qty, procurement.product_uom,
                                 procurement.location_id, procurement.name + '-mto', procurement.origin, procurement.company_id,
                                 procurement.values)

                getattr(rule.mto_rule_id, '_run_%s' % rule.mto_rule_id.action)([(mto_procurement, rule.mto_rule_id)])
        return True
